...
const validatePage = (roles: Array<string>) => {
	cy.get(SELECT_CLIENT_ROLES).should('exist');
	cy.get(SELECT_CLIENT_ROLES_TITLE).should('have.text', 'Test Client');
	cy.get(SELECT_ADD_ROLE_BTN).should('have.text', 'Add Role');
	...
	if (roles.length > 0) {
		cy.get(SELECT_CLIENT_ROLES_LIST)
			...
			.each(($li, index) => {
				...
				cy.wrap($li).find('button').eq(0).should('have.text', 'Edit');
				cy.wrap($li).find('button').eq(1).should('have.text', 'Delete');
			});

		cy.get(SELECT_NO_ROLES_MSG).should('not.exist');
	} else {
		cy.get(SELECT_CLIENT_ROLES_LIST).should('not.exist');
		cy.get(SELECT_NO_ROLES_MSG).should('have.text', 'No Roles');
	}
};
...
const validateRoleDialog = (text: string) => {
	cy.get(SELECT_ROLE_DIALOG).should('exist');
	cy.get(SELECT_ROLE_DIALOG).find('input').should('have.value', text);
};

const typeRoleInDialog = (text: string) => {
	cy.get(SELECT_ROLE_DIALOG).find('input').clear().type(text);
};

const clickRoleDialogSaveBtn = () => {
	cy.get(SELECT_ROLE_DIALOG).find('button').eq(0).click();
};

const clickRoleDialogCancelBtn = () => {
	cy.get(SELECT_ROLE_DIALOG).find('button').eq(1).click();
};
...
const validateDeleteRoleDialog = () => {
	cy.get(SELECT_DELETE_DIALOG).should('exist');
...
const clickDeleteConfirmBtn = () => {
	cy.get(SELECT_DELETE_DIALOG).find('button').eq(0).click();
};

const clickDeleteCancelBtn = () => {
	cy.get(SELECT_DELETE_DIALOG).find('button').eq(1).click();
};
