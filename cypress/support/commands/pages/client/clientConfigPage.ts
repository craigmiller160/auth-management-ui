...
const SELECT_REFRESH_TIME_FIELD =
	'#client-config-page #refresh-token-time-field';
...
const SELECT_GEN_CLIENT_KEY_BTN =
	'#client-config-page #client-key-generate-btn';
const SELECT_GEN_CLIENT_SECRET_BTN =
	'#client-config-page #client-secret-generate-btn';

const SELECT_SAVE_BTN = '#client-config-page #save-btn';
const SELECT_DELETE_BTN = '#client-config-page #delete-btn';

const SELECT_ENABLED_LABEL =
	'#client-config-page .switch-label .MuiFormControlLabel-label';
const SELECT_CLIENT_NAME_LABEL =
	'#client-config-page label[for="client-name-field"]';
const SELECT_CLIENT_KEY_LABEL =
	'#client-config-page label[for="client-key-field"]';
const SELECT_CLIENT_SECRET_LABEL =
	'#client-config-page label[for="client-secret-field"]';
const SELECT_ACCESS_TIME_LABEL =
	'#client-config-page label[for="access-token-time-field"]';
const SELECT_REFRESH_TIME_LABEL =
	'#client-config-page label[for="refresh-token-time-field"]';
const SELECT_CODE_TIME_LABEL =
	'#client-config-page label[for="auth-code-time-field"]';
...
const validateClientConfigCommon = (newClient = false) => {
	cy.get(SELECT_GEN_CLIENT_KEY_BTN).should('have.text', 'Generate');
	cy.get(SELECT_GEN_CLIENT_SECRET_BTN).should('have.text', 'Generate');
	cy.get(SELECT_CLIENT_NAME_LABEL).should('have.text', 'Client Name');
	cy.get(SELECT_CLIENT_KEY_LABEL).should('have.text', 'Client Key');
	cy.get(SELECT_CLIENT_SECRET_LABEL).should('have.text', 'Client Secret');
	...
	cy.get(SELECT_SAVE_BTN).should('have.text', 'Save');
	if (!newClient) {
		cy.get(SELECT_DELETE_BTN).should('have.text', 'Delete');
	}
...
const validateClientConfigValues = (values: ClientConfigValues) => {
	cy.get(SELECT_CLIENT_NAME_FIELD).should('have.value', values.clientName);
	cy.get(SELECT_CLIENT_KEY_FIELD).should('not.have.value', '');
	cy.get(SELECT_CLIENT_SECRET_FIELD).should('not.have.value', '');
	...
	if (values.clientSecretHasPlaceholder) {
		cy.get(SELECT_CLIENT_SECRET_FIELD).should('have.value', '**********');
	} else {
		cy.get(SELECT_CLIENT_SECRET_FIELD).should(
			'not.have.value',
			'**********'
		);
	}
...
const generateFieldValue = (genFieldName: string) => {
	if (CLIENT_KEY === genFieldName) {
		...
		throw new Error(
			`Invalid field name to generate value: ${genFieldName}`
		);
	}
};

const setConfigValues = (values: ClientConfigValues) => {
	cy.get(SELECT_CLIENT_NAME_FIELD).clear().type(values.clientName);
	...
	cy.get(SELECT_CODE_TIME_FIELD).clear().type(`${values.authCodeTimeout}`);
