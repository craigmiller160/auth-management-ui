...
describe('ClientConfig', () => {
	...
	describe('rendering', () => {
		it('renders for new client', async () => {
			...
			expect(
				screen.getByLabelText('Access Token Timeout (Secs)')
			).toHaveValue(300);
			expect(
				screen.getByLabelText('Refresh Token Timeout (Secs)')
			).toHaveValue(3600);
			expect(
				screen.getByLabelText('Auth Code Timeout (Secs)')
			).toHaveValue(60);
		...
		it('renders for existing client', async () => {
			...
			expect(
				screen.getByText('https://www.google.com')
			).toBeInTheDocument();

			expect(
				screen.getByText('https://www.facebook.com')
			).toBeInTheDocument();
	...
	describe('behavior', () => {
		it('fills out editable fields', async () => {
			...
			const accessToken = screen.getByLabelText(
		...
		it('cancels deleting client', async () => {
			...
			await waitFor(() =>
				expect(
					screen.queryByText('Delete Client')
		...
		it('cancels adding redirect uri', async () => {
			...
			await waitFor(() =>
				expect(
					screen.queryByText('Redirect URI')
				).not.toBeInTheDocument()
			);
		});
		...
		it('edits redirect uri', async () => {
			...
			await waitFor(() => {
				...
				const otherListItem = screen.getByTestId(
					'redirect-uris-list-item-1'
				);
				const otherText = otherListItem.querySelector(
					'#redirect-uris-list-item-1-text-primary'
				);
				expect(otherText?.textContent).toEqual(
